...
namespace QUT.Gppg {
 ...
 where TSpan : IMerge<TSpan>, new() {
        ...
        public bool Parse() {
            ...
            while (true) {
#if TRACE_ACTIONS
                Console.WriteLine("Entering state {0} ", FsaState.number);
                DisplayStack();
#endif
...
#if TRACE_ACTIONS
                       Console.WriteLine( "Reading: Next token is {0}", TerminalToString( NextToken ) );
#endif
                    }
#if TRACE_ACTIONS
                    else
                        Console.WriteLine( "Next token is still {0}", TerminalToString( NextToken ) );
#endif
...
#if TRACE_ACTIONS
				Console.Write("Shifting token {0}, ", TerminalToString(NextToken));
#endif
...
#if TRACE_ACTIONS
					Console.WriteLine("Error: popping state {0}", StateStack.TopElement().number);
#endif
                StateStack.Pop();
                valueStack.Pop();
                LocationStack.Pop();
...
#if TRACE_ACTIONS
                        Console.WriteLine("Aborting: didn't find a state that accepts error token");
#endif
                    return false;
                }
                else
        ...
        private bool DiscardInvalidTokens() {
            ...
            if (FsaState.ParserTable != null) {
                ...
                while (true) {
                    if (NextToken == 0) {
#if TRACE_ACTIONS
                            Console.Write("Reading a token: ");
#endif
                        NextToken = scanner.yylex();
                    }
#if TRACE_ACTIONS
                        Console.WriteLine("Next token is {0}", TerminalToString(NextToken));
#endif
                    if (NextToken == endOfFileToken)
                        return false;
...
#if TRACE_ACTIONS
                            Console.WriteLine("Error: Discarding {0}", TerminalToString(NextToken));
#endif
                        NextToken = 0;
                    }
                }
                ...
                //
#if TRACE_ACTIONS
                    Console.WriteLine("Error: panic discard of {0}", TerminalToString(NextToken));
#endif
                if (NextToken == endOfFileToken)
                    return false;
                NextToken = 0;
        ...
        private void DisplayStack() {
            Console.Write( "State stack is now:" );
            for (int i = 0; i < StateStack.Depth; i++)
                Console.Write( " {0}", StateStack[i].number );
            Console.WriteLine();
        }

        private void DisplayRule( int ruleNumber ) {
            Console.Write( "Reducing stack by rule {0}, ", ruleNumber );
        ...
        private void DisplayProduction( Rule rule ) {
            if (rule.RightHandSide.Length == 0)
                ...
                foreach (int symbol in rule.RightHandSide)
                    Console.Write( "{0} ", SymbolToString( symbol ) );

            Console.WriteLine( "-> {0}", SymbolToString( rule.LeftHandSide ) );
        }
