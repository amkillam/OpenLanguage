...
%x IN_COMMENT_FROM_INITIAL
%x IN_COMMENT_FROM_SR_POSSIBLE
%x SR_POSSIBLE
...
<SR_POSSIBLE>{
    ...
    "/*" { BEGIN(IN_COMMENT_FROM_SR_POSSIBLE); }
...
<IN_COMMENT_FROM_INITIAL>"\*/" { BEGIN(INITIAL); }
<IN_COMMENT_FROM_INITIAL><<EOF>> { throw new System.FormatException("Unterminated comment"); }
<IN_COMMENT_FROM_INITIAL>[\s\S] { /* consume comment content */ }

<IN_COMMENT_FROM_SR_POSSIBLE>"\*/" { BEGIN(SR_POSSIBLE); }
<IN_COMMENT_FROM_SR_POSSIBLE><<EOF>> { throw new System.FormatException("Unterminated comment"); }
<IN_COMMENT_FROM_SR_POSSIBLE>[\s\S] { /* consume comment content */ }
...
<IN_A1_CELL> {
    ...
    [1-9][0-9]{0,6} { BEGIN(SR_POSSIBLE); yylval.ulongVal = ulong.Parse(yytext, System.Globalization.CultureInfo.InvariantCulture); if (yylval.ulongVal > 1048576UL || yylval.ulongVal < 1) { throw new System.FormatException("Row number out of range"); } return (int)Tokens.T_A1_ROW; }
    [0]             { BEGIN(SR_POSSIBLE); yyless(0); throw new System.FormatException("Row numbers cannot start with 0"); }
    .               { BEGIN(SR_POSSIBLE); yyless(0); }
}
...
<IN_A1_COLUMN_RANGE_SECOND_COLUMN> {
   ...
   [A-Z][A-Z]{0,2}                                                    { BEGIN(SR_POSSIBLE); yylval.ulongVal = AlphabeticHexevigesimalProvider.Parse<ulong>(yytext); if (yylval.ulongVal > 16384UL) { throw new System.FormatException("Column reference out of range"); } return (int)Tokens.T_A1_COLUMN; }
   . { BEGIN(SR_POSSIBLE); yyless(0); }
}
...
<IN_A1_ROW_RANGE> {
   ...
   [0]                                                          { yyless(0); BEGIN(SR_POSSIBLE); throw new System.FormatException("Row numbers cannot start with 0"); }
...
"]" { BEGIN(SR_POSSIBLE); yylval.stringVal = yytext; return (int)Tokens.T_RBRACK; }
":" { yylval.stringVal = yytext; BEGIN(SR_POSSIBLE); return (int)Tokens.T_COLON; }
...
[\+\-]?[1-9][0-9]* { yylval.longVal = long.Parse(yytext, System.Globalization.CultureInfo.InvariantCulture); return (int)Tokens.T_R1C1_COLUMN; }
"]" { BEGIN(SR_POSSIBLE); return (int)Tokens.T_RBRACK; }
...
<IN_WBREF>{
    ...
    [\r][\n] { yylval.stringVal = yytext; BEGIN(SR_POSSIBLE); return (int)Tokens.T_NEWLINE; }
    [\n]     { yylval.stringVal = yytext; BEGIN(SR_POSSIBLE); return (int)Tokens.T_NEWLINE; }
    [\r]     { yylval.stringVal = yytext; BEGIN(SR_POSSIBLE); return (int)Tokens.T_NEWLINE; }
}
...
<IN_SR_COLUMN>{
  // Quoted column names are valid in SRs
  \'([^\']|\'\')*\' { yylval.stringVal = yytext; return (int)Tokens.T_IDENTIFIER; }

  // Manage nested brackets and exit at the last one
  \[ {
      yylval.stringVal = yytext;
  ...
  // Structured Reference keywords inside brackets (must win over identifiers)
  ...
  // Column names: one name -> one identifier (tick-escapes allowed, interior spaces preserved)
  {SR_COL_NAME} {
  ...
  // Column identifier run that includes interior spaces (non-space start, non-space end)
  ...
  // Punctuation inside SR brackets
  ...
  // Fallback identifier for plain column names that didn’t match SR_COL_NAME (e.g., Column1, Column_1)
  ...
  // Close bracket; only leave state when the outermost bracket closes
  \] {
      yylval.stringVal = yytext;
      sr_bracket_nesting_level--;
