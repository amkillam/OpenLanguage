using System;
using System.Collections.Generic;
using System.Linq;

namespace OpenLanguage.WordprocessingML.FieldInstruction.Typed
{
    /// <summary>
    /// Represents a strongly-typed FILENAME field instruction.
    /// Retrieves the name of the current document as stored on disk.
    /// </summary>
    public class FileNameFieldInstruction : TypedFieldInstruction
    {
        /// <summary>
        /// Switch: \p
        /// Include the full file path name.
        /// </summary>
        public bool IncludeFullPath { get; set; }

        /// <summary>
        /// Initializes a new instance of the FileNameFieldInstruction class.
        /// </summary>
        /// <param name="source">The source FieldInstruction to parse.</param>
        public FileNameFieldInstruction(FieldInstruction source)
            : base(source)
        {
            ParseArguments();
        }

        /// <summary>
        /// Parses the arguments from the source FieldInstruction.
        /// </summary>
        private void ParseArguments()
        {
            if (Source.Instruction.ToUpperInvariant() != "FILENAME")
            {
                throw new ArgumentException(
                    $"Expected FILENAME field, but got {Source.Instruction}"
                );
            }

            // FILENAME has no field arguments, only switches
            List<FieldArgument> nonSwitchArgs = Source
                .Arguments.Where(arg => arg.Type != FieldArgumentType.Switch)
                .ToList();

            if (nonSwitchArgs.Count > 0)
            {
                throw new ArgumentException("FILENAME field takes no arguments, only switches");
            }

            // Parse switches
            for (int i = 0; i < Source.Arguments.Count; i++)
            {
                FieldArgument arg = Source.Arguments[i];
                if (arg.Type == FieldArgumentType.Switch)
                {
                    string switchValue = arg.Value?.ToString() ?? string.Empty;
                    switch (switchValue.ToLowerInvariant())
                    {
                        case "p":
                            IncludeFullPath = true;
                            break;
                    }
                }
            }
        }

        /// <summary>
        /// Reconstructs the field instruction as a string.
        /// </summary>
        /// <returns>The field instruction string.</returns>
        public override string ToString()
        {
            List<string> result = new List<string> { "FILENAME" };

            if (IncludeFullPath)
            {
                result.Add("\\p");
            }

            return string.Join(" ", result);
        }
    }
}
